...
class OpenAIClient {
  ...
  async generateContent(prompt: string, userInfo: any): Promise<string> {
    // 優先度順にプロキシサービスを試行
  ...
  private async tryProxyService(service: any, prompt: string): Promise<string> {
  ...
  generatePrompt(userInfo: any): string {
    return `
